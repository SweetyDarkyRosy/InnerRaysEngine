...
void CControlPathBuilder::update_schedule()
{
	...
	// The one and only reason is because of the restriction-change, so wait until position and node will be in valid state
...
u32 CControlPathBuilder::find_nearest_vertex(const u32& level_vertex_id, const Fvector& target_position, const float& range)
{
	...
	ai().graph_engine().search(ai().level_graph(), level_vertex_id, level_vertex_id, &temp, GraphEngineSpace::CNearestVertexParameters(
