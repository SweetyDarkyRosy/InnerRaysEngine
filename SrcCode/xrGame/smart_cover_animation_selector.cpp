...
void animation_selector::initialize()
{
	...
	/**
		We need this, since we could change bone callbacks which needs animation blends but blends are not accessible since
		we didn't called animation().update() which is called on UpdateCL and in case of RayQuery our bone callback could be called
		but UpdateCL is not called yet and we will have
	 */
