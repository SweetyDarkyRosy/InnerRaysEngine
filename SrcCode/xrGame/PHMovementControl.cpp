...
void CPHMovementControl::PathNearestPoint(const xr_vector<DetailPathManager::STravelPathPoint>& path, const Fvector& new_position,
	int& index, bool& near_line)
{
...
void CPHMovementControl::PathNearestPointFindUp(const xr_vector<DetailPathManager::STravelPathPoint>& path,
	const Fvector& new_position, int& index, float radius, bool& near_line)
{
	Fvector from_first, from_second, dir;
	bool after_line = true;//to check first point
...
void CPHMovementControl::PathNearestPointFindDown(const xr_vector<DetailPathManager::STravelPathPoint>& path, const Fvector& new_position,
	int& index, float radius, bool& near_line)
{
	Fvector from_first, from_second, dir;
	bool after_line = true;													// To check first point
...
void CPHMovementControl::PathDIrPoint(const xr_vector<DetailPathManager::STravelPathPoint>& path, int index, float distance, float precesition, Fvector& dir)
{
	...
	{
		if (0 == index || m_path_size - 1 == index)							// On path edge
		{
			dir.set(corrected_path_dir);
			return;
		}
	...
	to_path_point.mul(1.f / mag);
	if (m_path_size - 1 == index)											// On path edge
	{
...
void CPHMovementControl::Load(LPCSTR section)
{
	...
	bb.set(vFOOT_center, vFOOT_center);
	bb.grow(vFOOT_size);
...
void CPHMovementControl::CheckEnvironment(const Fvector&)
{
...
void CPHMovementControl::ApplyHit(const Fvector& dir, const dReal P, ALife::EHitType hit_type)
{
	...
	if (Environment() == CPHMovementControl::peOnGround || Environment() == CPHMovementControl::peAtWall)
	{
		switch (hit_type)
		{
			...
			case ALife::eHitTypeWound_2:			// Knife's alternative fire
...
void CPHMovementControl::UpdateObjectBox(CPHCharacter* ach)
{
	...
	Fvector2 plane_cam; plane_cam.set(Device.vCameraDirection.x, Device.vCameraDirection.z);
