...
void game_cl_Deathmatch::SetBuyMenuItems(PRESET_ITEMS* pItems, BOOL OnlyPreset)
{
	...
	if (pCurActor)
	{
		...
		aditional_ammo_t add_ammo(_alloca(sizeof(aditional_ammo_t::value_type) * (max_addammo_count * 2)),
			(max_addammo_count * 2));
		...
		std::for_each(add_ammo.begin(), add_ammo.end(),
			std::bind1st(std::mem_fun<void, game_cl_Deathmatch, aditional_ammo_t::value_type const &>(&game_cl_Deathmatch::AdditionalAmmoInserter),
				this));
		*/
