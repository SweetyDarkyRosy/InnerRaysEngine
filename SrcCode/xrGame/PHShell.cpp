...
void CPHShell::AddElementRecursive(CPhysicsElement* root_e, u16 id, Fmatrix global_parent, u16 element_number, bool* vis_check)
{
	...
	bool breakable = joint_data.ik_flags.test(SJointIKData::flBreakable) &&
		root_e && !(no_physics_shape(bone_data.shape) && joint_data.type == jtRigid);
...
// Настраивает фильтр коллизий на игнорирование столкновенний данного физического объекта с анимированным физическим объектом
void CPHShell::SetIgnoreAnimated()
{
	// Для фильтра коллизий указываем, что данный физический объект игнорирует анимированные физические тела
