...
void CSE_ALifeGroupAbstract::try_switch_offline()
{
	...
	// So, we have a group of objects. Therefore check all the group members if they are ready to switch offline
	...
	for (u32 i = 0, N = (u32)m_tpMembers.size(); i < N; ++i)
	{
		...
		{
			// So, monster is not dead. Checking if the object is _not_ ready to switch offline
			...
			{
				// So, it is not ready, breaking a cycle, because we can't switch group offline since not all the group members are ready to switch offline
				break;
			}
