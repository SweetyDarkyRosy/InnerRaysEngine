...
#define CBucketList CDataStorageBucketList<_path_id_type, _bucket_id_type, bucket_count,	\
...
IC bool CBucketList::is_opened_empty()
{
	...
	if (!m_buckets[m_min_bucket_id])
	{
		if (!clear_buckets)
		{
			for (++m_min_bucket_id; (m_min_bucket_id < bucket_count) && (!m_buckets[m_min_bucket_id] || (m_buckets[m_min_bucket_id]->m_path_id != current_path_id()) ||
				(m_buckets[m_min_bucket_id]->m_bucket_id != m_min_bucket_id)); ++m_min_bucket_id);
		}
		else
		{
