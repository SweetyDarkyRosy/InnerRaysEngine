...
void CUIMainIngameWnd::Update()
{
	...
	{
		// This is a bad style... It left for backward compatibility. Need to move this logic into UIGameCTA class
	...
	while (i <= ewiWeaponJammed) // ewiInvincible
	{
		...
		{
			rit = std::find_if(m_Thresholds[i].rbegin(), m_Thresholds[i].rend(), [&value](float a)
				{
					return a < value;
				});
		}
...
void CUIMainIngameWnd::UpdatePickUpItem()
{
	...
	UIPickUpItemIcon.GetUIStaticItem().SetOriginalRect(float(m_iXPos * INV_GRID_WIDTH), float(m_iYPos * INV_GRID_HEIGHT),
