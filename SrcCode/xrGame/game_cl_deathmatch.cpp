...
void game_cl_Deathmatch::shedule_Update(u32 dt)
{
	...
	if ((Level().IsDemoPlayStarted() || Level().IsDemoPlayFinished()) && m_game_ui)
	{
		...
		STRCONCAT(demo_play_string, "demo play active : ", itoa(int(Level().GetDemoPlayPos() * 100), tmp_buf1, 10),
			"%%, play speed: ", itoa(int(Level().GetDemoPlaySpeed()), tmp_buf2, 10), "x");
			...
			if (local_player && !local_player->IsSkip())
			{
				...
				if (Level().CurrentEntity() && smart_cast<CSpectator*>(Level().CurrentEntity()))
				{
					if (!(pCurBuyMenu && pCurBuyMenu->IsShown()) && !(pCurSkinMenu && pCurSkinMenu->IsShown()) &&
						!(m_game_ui->m_pMapDesc && m_game_ui->m_pMapDesc->IsShown()) && (HUD().GetUI() && HUD().GetUI()->GameIndicatorsShown()))
					{
						if (!m_bSkinSelected)
						{
