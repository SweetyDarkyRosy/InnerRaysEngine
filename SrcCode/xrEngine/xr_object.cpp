...
// Constructor
CObject::CObject() :
	ISpatial(g_SpatialSpace), dwFrame_AsCrow(u32(-1))
{
	// Transform
	Props.storage = 0;
...
BOOL CObject::net_Spawn(CSE_Abstract* data)
{
	...
	if (Visual() == 0 && pSettings->line_exist(cNameSect(), "visual"))
	{
		cNameVisual_set(pSettings->r_string(cNameSect(), "visual"));
	}

	if (collidable.model == 0)
	{
