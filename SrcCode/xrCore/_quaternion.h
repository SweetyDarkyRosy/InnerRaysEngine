...
public:
	...
	// Multiplies q1 * q2, and places the result in *this. No failure. Renormalization not automatic
	...
	*/
	...
	/**
		...
		Returns TRUE if there is an axis. Returns FALSE if there is no axis (and Axis is set to 0,0,0, and Theta is 0)
	 */
	...
	/**
		...
		Resulting quaternion is 'between' q0 and q1 with t == 0 being all q0, and t == 1 being all q1.
		Returns a quaternion with a positive W - always takes shortest route through the positive W domain.
	 */
	...
	ICF SelfRef slerp(SelfCRef Q0, SelfCRef Q1, T tm)
	{
		...
		{
			// Has numerical difficulties around cosom == 0. In this case degenerate to linear interpolation
			Scale0 = 1.0f - tm;
			Scale1 = tm;
		}
	...
	// Returns TRUE if quaternions differ elementwise by less than Tolerance.
	IC BOOL cmp(SelfCRef Q, T Tolerance = 0.0001f)
	{
		if (((_abs(x + Q.x) <= Tolerance)			// They are the same but with opposite signs
