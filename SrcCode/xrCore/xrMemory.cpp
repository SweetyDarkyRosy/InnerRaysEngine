...
void xrMemory::mem_statistic(LPCSTR fn)
{
	...
	for (u32 it = 0; it < debug_info.size(); it++)
	{
		...
		try
		{
			if (strcmp(debug_info[it]._name,"storage: sstring") == 0)
			{
	...
	*/
	...
	if (g_pStringContainer)
	{
		Msg("memstat: shared_str: economy: %d bytes", g_pStringContainer->stat_economy());
	}

	if (g_pSharedMemoryContainer)
	{
		Msg("memstat: shared_mem: economy: %d bytes", g_pSharedMemoryContainer->stat_economy());
	}
	...
	// Output to log
	{
		...
		for ( ; I != E; I++)
		{
			Msg("%8d : %-4d [%d]", I->first, I->second, I->first * I->second);
		}
	}
	*/
}
