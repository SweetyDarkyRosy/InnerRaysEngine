#ifndef __INNER_RAYS_ENGINE_BITWISE_H__
#define __INNER_RAYS_ENGINE_BITWISE_H__
#pragma once

/*[
	Floating-point value defines
 ]*/
...
// Here are a few nice tricks for 2's complement based machines that I discovered a few months ago

IC int btwLowestBitMask(int v) { return (v & -v); }
IC u32 btwLowestBitMask(u32 x) { return x & ~(x - 1); }

// Ok, so now we are cooking on gass. Here we use this function for some rather useful utility functions
...
// Couple more tricks. Counting number of nonzero bits for 8bit number: 
...
// intCeil() is a non-interesting variant, since effectively. ceil(x) == -floor(-x)
...
#endif // !__INNER_RAYS_ENGINE_BITWISE_H__
