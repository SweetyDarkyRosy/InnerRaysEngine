...
HRESULT CRender::shader_compile(LPCSTR name, LPCSTR pSrcData, UINT SrcDataLen, void* _pDefines, void* _pInclude,
	LPCSTR pFunctionName, LPCSTR pTarget, DWORD Flags, void* _ppShader, void* _ppErrorMsgs, void* _ppConstantTable)
{
	...
	if (0 == xr_strcmp(pFunctionName, "main"))
	{
		if ('v' == pTarget[0])
		{
			if (HW.pDevice1 == 0)
			{
				pTarget = D3D10GetVertexShaderProfile(HW.pDevice);
			}
			else
			{
				pTarget = "vs_4_1";
		...
		else if ('p' == pTarget[0])
		{
			if (HW.pDevice1 == 0)
			{
				pTarget = D3D10GetPixelShaderProfile(HW.pDevice);
			}
			else
			{
				pTarget = "ps_4_1";
		...
		else if ('g' == pTarget[0])
		{
			if (HW.pDevice1 == 0)
			{
				pTarget = D3D10GetGeometryShaderProfile(HW.pDevice);
			}
			else
			{
				pTarget = "gs_4_1";
	...
	HRESULT _result = D3DX10CompileFromMemory(pSrcData, SrcDataLen, "",
		defines, pInclude, pFunctionName, pTarget, Flags, 0, NULL, ppShader, ppErrorMsgs, NULL);
